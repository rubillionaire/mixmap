...
function Map (rcom, opts) {
  ...
  this._clearOpts = defined(opts.clear, { depth: true, color: this.backgroundColor })
  this._mouse = null
  this._size = null
  this.size = null
  this.on('viewbox', this._onviewbox)
  this.on('resize', () => this._pickInit = false)
}
...
Map.prototype._onviewbox = function () {
  ...
  self.emit('viewbox:internal:start')
  function ready () {
    self._idle = null
    ...
    function done () {
      if (self._viewboxpending > 1) {
        ...
        self._viewboxpending = 0
        self.emit('viewbox:internal:end')
      }
    }
...
Map.prototype.draw = function () {
  this.regl.poll()
  this.regl.clear(this._clearOpts)
...
Map.prototype.pick = function (opts, cb) {
  ...
  const fbWidth = defined(opts.fbWidth, self._size[0])
  const fbHeight = defined(opts.fbHeight,self._size[1])
  var x = Math.max(0, Math.min(fbWidth-1, ex))
  var y = Math.max(0, Math.min(fbHeight-1, fbHeight-ey))
  var first = false
  if (!self._fb) {
  ...
  if (self._pickInit === false) {
    self._pickInit = true
    self._fb.resize(fbWidth, fbHeight)
    self._fb.use(function () {
      var props = self._props()
      for (var i = 0; i < props.length; i++) {
        Object.assign(props[i], defined(opts.props, {}))  
      }
      for (var i = 0; i < self._draw.length; i++) {
...
Map.prototype.render = function (props) {
  ...
  if (typeof props.attributes === 'object' && props.attributes !== null) {
    Object.assign(
      attributes,
      props.attributes,
      { style: Object.assign(attributes.style, props.attributes.style ?? {}) },
      { class: `${attributes.class} ${props.attributes.class ?? ''}`})
  }
